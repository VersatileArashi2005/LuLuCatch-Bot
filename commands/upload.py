...
# We'll use pool from application.bot_data
...
async def callback_router(update: Update, context):
    ...
    if m:
        ...
        pending_uploads[user.id] = st
        await query.edit_message_text("📷 Please send the card photo now (in this private chat).")
        return
...
async def photo_handler(update: Update, context):
    ...
    anime = st["anime"]; character = st["character"]; rarity = st["rarity"]
